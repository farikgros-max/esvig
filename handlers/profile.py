...
from keyboards import cancel_keyboard, get_main_keyboard, get_profile_keyboard
...
async def confirm_cancel(cb: CallbackQuery):
    ...
    if ordd['status'] not in ('в обработке', 'оплачена', 'ожидает оплаты'):
        ...
        return
    refunded = await refund_cancelled_order(ordd)
    await update_order_status(order_id, 'отменена')
    if refunded:
        try:
            await cb.bot.send_message(
                ordd['user_id'],
                f"📢 Ваша заявка #{order_id} отменена. Средства возвращены на баланс.",
            )
        except Exception:
            pass
    msg = "Заявка отменена, деньги возвращены на баланс" if refunded else "Заявка отменена"
    await cb.answer(msg, False)
    if refunded:
        for aid in ADMIN_IDS:
            try:
                await cb.bot.send_message(
                    aid,
                    f"❌ Заявка #{order_id} отменена пользователем @{ordd['username']} "
                    f"(сумма {ordd['total']}$). Деньги возвращены.",
                )
            except Exception:
                pass
    await my_ords(cb)

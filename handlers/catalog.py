...
from database import get_all_channels, get_all_categories, get_category_by_id
...
async def add_to_cart(cb: CallbackQuery):
    ...
    if not info:
        await cb.answer("Канал не найден", True)
        return
    # глобальная корзина пока останется в этом модуле? перенесём её в cart.py, но функция get_cart вызывается из каталога, проще оставить словарь в cart.py и импортировать

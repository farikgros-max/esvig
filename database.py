...
async def get_user_invited_count(user_id: int) -> int:
    conn = await get_connection()
    return (await conn.fetchval('SELECT COUNT(*) FROM users WHERE inviter_id = $1', user_id)) or 0

def get_referral_level(invited_count: int) -> int:
    if invited_count <= 10: return 1
    elif invited_count <= 50: return 2
    else: return 3

def get_referral_percent(level: int) -> float:
    if level == 1: return 1.0
    elif level == 2: return 2.0
    elif level == 3: return 3.5
    return 0.0
...
async def get_referral_stats(user_id: int):
    ...
    conn = await get_connection()
    bonuses = (await conn.fetchval("SELECT COALESCE(SUM(amount),0) FROM transactions WHERE user_id = $1 AND type = 'реферальный бонус'", user_id)) or 0
    next_level = level + 1 if level < 3 else None
    needed = 0
    if next_level == 2: needed = 11 - invited
    elif next_level == 3: needed = 51 - invited
    return {"invited": invited, "level": level, "percent": percent, "bonuses": bonuses, "next_level": next_level, "needed": needed}
...
async def process_referral_bonus(order_id: int, order_total: int):
    ...
    if not inviter or not inviter['inviter_id']: return
    ...
    async with conn.transaction():
        ...
        await conn.execute("INSERT INTO transactions (user_id, type, amount, order_id, description) VALUES ($1, 'реферальный бонус', $2, $3, $4)",
                           inviter_id, bonus, order_id, f"Реферальный бонус ({percent}%) от заказа #{order_id} пользователя {user_id}")
...
async def process_pending_orders(user_id: int):
    conn = await get_connection()
    pending = await conn.fetch("SELECT id, total FROM orders WHERE user_id = $1 AND status = 'ожидает оплаты' ORDER BY id", user_id)
...
async def get_channels(category_id=None): return await get_all_channels(category_id)
...
async def delete_channel(ch_id):
    ...
    if ch_id in _channels_dict: del _channels_dict[ch_id]
...
async def get_seller_applications(status: str = 'pending') -> list:
    conn = await get_connection()
    rows = await conn.fetch('''SELECT id, user_id, username, channel_url, channel_name, price, description, status, created_at

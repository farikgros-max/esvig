import os, asyncio, logging, hashlib, hmac, json
...
from database import init_db, update_user_balance, auto_process_orders, get_connection, close_db, backup_database, cleanup_old_slots
...
# ВРЕМЕННЫЙ ГЛОБАЛЬНЫЙ ПЕРЕХВАТЧИК
@dp_instance.errors()
async def global_error_handler(event, exception):
    import traceback
    print("!!! ГЛОБАЛЬНАЯ ОШИБКА !!!")
    traceback.print_exc()
    # Попробуем отправить сообщение пользователю, если возможно
    if hasattr(event, 'answer'):
        try:
            await event.answer(f"❌ Ошибка: {exception}")
        except:
            pass
...
async def startup():
    ...
    import handlers.admin_extended  # noqa: F401 — регистрирует обработчики на admin router
    ...
    dp_instance.message.middleware(sub_mw)
    dp_instance.callback_query.middleware(sub_mw)
    dp_instance.message.middleware(flood_mw)
    dp_instance.callback_query.middleware(flood_mw)
...
async def periodic_slot_cleanup():
    while True:
        await asyncio.sleep(86400)
        try:
            await cleanup_old_slots()
            print("[SLOTS] Старые слоты очищены")
        except Exception as e:
            print(f"[SLOTS] Ошибка очистки слотов: {e}")

async def main():
    await startup()
    asyncio.create_task(auto_process_orders(bot_instance))
    asyncio.create_task(db_health_check())
    asyncio.create_task(periodic_slot_cleanup())
    ...
    await site.start()
    print(f"Платёжный HTTP-сервер запущен на порту {os.environ.get('PORT', 8443)}")

...
class AddChannelStates(StatesGroup):
    waiting_for_category = State()   # сначала категория
...
def get_category_selection_keyboard():
    ...
    for i in range(0, len(cats), 2):
        row = []
        row.append(InlineKeyboardButton(text=cats[i]['display_name'], callback_data=f"add_chan_cat_{cats[i]['id']}"))
        if i+1 < len(cats):
            row.append(InlineKeyboardButton(text=cats[i+1]['display_name'], callback_data=f"add_chan_cat_{cats[i+1]['id']}"))
        rows.append(row)
    rows.append([InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def get_edit_category_selection_keyboard(cid):
    cats = get_all_categories()
    if not cats:
        return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"admin_view_{cid}")]])
    rows = []
    for i in range(0, len(cats), 2):
        row = []
        row.append(InlineKeyboardButton(text=cats[i]['display_name'], callback_data=f"edit_chan_cat_{cid}_{cats[i]['id']}"))
        if i+1 < len(cats):
            row.append(InlineKeyboardButton(text=cats[i+1]['display_name'], callback_data=f"edit_chan_cat_{cid}_{cats[i+1]['id']}"))
        rows.append(row)
...
async def register_handlers(dp: Dispatcher):
    ...
    async def select_category(cb: CallbackQuery):
        ...
        cat_name = cat['display_name'] if cat else "Категория"
        await cb.message.edit_text(f"📢 {cat_name} (страница 1/{total})", reply_markup=kb)
        await cb.answer()
    ...
    async def view_catalog_page(cb: CallbackQuery):
        ...
        if kb:
            cat = get_category_by_id(cat_id)
            cat_name = cat['display_name'] if cat else "Категория"
            await cb.message.edit_text(f"📢 {cat_name} (страница {cur+1}/{total})", reply_markup=kb)
        else:
    ...
    async def adm_add_start(cb: CallbackQuery, state: FSMContext):
        if cb.from_user.id not in ADMIN_IDS: await cb.answer("Нет прав", True); return
        await cb.message.edit_text("➕ Добавление канала\n\nВыберите категорию:", reply_markup=get_category_selection_keyboard())
    ...
    async def add_channel_category_selected(cb: CallbackQuery, state: FSMContext):
        ...
        if not cat:
            await cb.answer("Категория не найдена", True)
            return
        await state.update_data(category_id=cat_id, category_name=cat['display_name'])
    ...
    async def a_desc(m: Message, state: FSMContext):
        ...
        add_channel(new_id, data['name'], data['price'], data['subscribers'], data['url'], data['description'], data['category_id'])
        await load_channels()
        await m.answer(
            f"✅ Канал {data['name']} добавлен в категорию {data['category_name']}!",
